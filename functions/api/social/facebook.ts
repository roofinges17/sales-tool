...
//
// Verification: X-Hub-Signature-256: sha256=<HMAC-SHA256 of body with META_APP_SECRET>
// Payload shape: { object: "page", entry: [{ changes: [{ field: "leadgen", value: { leadgen_id, page_id, form_id } }] }] }
//
// Meta only delivers leadgen_id — contact fields require a second Graph API call.
// Until META_PAGE_ACCESS_TOKEN is provisioned, leads are stored as status='pending_lead_fetch'.
// GET handler: Meta webhook verification challenge (hub.challenge).
...
export async function onRequestPost(ctx: { request: Request; env: Env }) {
  ...
  if (payload.object !== "page") {
    return Response.json({ skipped: "not_page_object" }, { headers: CORS });
  }
  ...
  for (const entry of payload.entry ?? []) {
    for (const change of entry.changes ?? []) {
      ...
      const result = await processSocialLead(env, {
        ...
        ...leadFields,
        isPending,
        rawPayload: { leadgen_id, page_id, form_id, entry_id: entry.id },
      });
