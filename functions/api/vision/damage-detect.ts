...
// Analyzes roof photos for damage using Gemini Flash Vision.
// Falls back to a realistic mock when GEMINI_API_KEY is not configured.
//
...
// ── Mock response for when GEMINI_API_KEY is not configured ──────────────────
...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  ...
  try {
    ...
    if (!geminiRes.ok) {
      ...
      console.error("[damage-detect] Gemini error:", geminiRes.status, errText);
      return new Response(
    ...
    console.error("[damage-detect] fetch error:", err);
